...
function Header() {
  ...
  return (
    <nav className={`sm:flex sm:flex-col sm:content-center z-50 sticky top-0 right-0 left-0 sm:h-[70px] ${expandedHeader ? "h-[300px]" : ""} md:bg-[#f1f6f9a9] backdrop-blur`}>
      <div className={`flex sm:flex-row sm:justify-around items-center bg-transparent h-full ${expandedHeader ? "flex-col justify-start items-center gap-y-[2px] pt-[8px]" : "justify-between p-2"}`}>
        ...
        <div className={`${expandedHeader ? "flex flex-col gap-y-[4px]" : "hidden"} sm:flex sm:flex-row justify-center items-center gap-x-5 `} id="mid-section">
          {navbarContent.map( (item) => (
            item.active ? <NavLink key={item.name} to={item.slug} className={ ({isActive}) => {return `${isActive ? "text-[#000000]" : ""} sm:text-xl text-base font-bold text-[#00000099] sm:hover:text-black transition-all duration-250 hover:transition-all hover:duration-250` } } >{item.name}</NavLink> : null
          ) )}
        </div>
